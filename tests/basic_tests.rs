...
async fn test_subscription_creation() {
    ...
    assert_eq!(subscription.state(), tokio_centrifuge::subscription::State::Unsubscribed);
}
...
async fn test_subscription_management() {
    ...
    assert_eq!(sub1.state(), tokio_centrifuge::subscription::State::Unsubscribed);
    assert_eq!(sub2.state(), tokio_centrifuge::subscription::State::Unsubscribed);
...
async fn test_config_protocols() {
    ...
    assert_eq!(json_config.protocol, tokio_centrifuge::config::Protocol::Json);
    
    // Test Protobuf protocol
    let proto_config = Config::new().use_protobuf();
    assert_eq!(proto_config.protocol, tokio_centrifuge::config::Protocol::Protobuf);
}
...
async fn test_error_handling() {
    ...
    assert_eq!(error.code, tokio_centrifuge::errors::ClientErrorCode::BadRequest);
    
    let error = ClientError::unauthorized("Unauthorized");
    assert_eq!(error.code, tokio_centrifuge::errors::ClientErrorCode::Unauthorized);
    
    let error = ClientError::internal("Internal error");
    assert_eq!(error.code, tokio_centrifuge::errors::ClientErrorCode::Internal);
}
...
async fn test_config_default_values() {
    ...
    assert_eq!(config.protocol, tokio_centrifuge::config::Protocol::Protobuf);
